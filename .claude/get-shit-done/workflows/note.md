...
- **Global scope**: `/Users/mc/Repos/sf-local-logs/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/mc/Repos/sf-local-logs/.claude/notes/`)
...
2. Glob `/Users/mc/Repos/sf-local-logs/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/mc/Repos/sf-local-logs/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/mc/Repos/sf-local-logs/.claude/notes/` — works in any directory
